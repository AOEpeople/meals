...
Mealz.prototype.enablePaypal = function () {
    ...
    // Only show PayPal buttons when the according radio button is chosen
    $('#ecash_paymethod input').on('click', function (event) {
        if (event.target.id === 'ecash_paymethod_0') {
            renderPaypalButtons();
        } else {
    ...
    /*
     * Render PayPal buttons and handle user input and requests
     */
    function renderPaypalButtons() {
        if ($('.paypal-buttons').length === 0) {
