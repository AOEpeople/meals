<template>
    <DisclosurePanel class="bg-white xl:hidden" v-slot="{ close }">
      <div class="grid border-t-2 border-gray-200 sm:grid-cols-2">
        ...
        <div class="pt-4 pb-3 border-t border-gray-200 sm:border-l sm:border-t-0">
          ...
          <span class="block px-4 py-2 text-base font-medium text-gray-600 cursor-pointer hover:text-highlight hover:bg-gray-100" @click="changeLocale">
            {{ t('changeLanguage') }}
          </span>
          <router-link
            @click="close()"
          ...
          <DisclosureButton as="a" href="/logout" class="block px-4 py-2 text-base font-medium text-gray-600 capitalize hover:text-highlight hover:bg-gray-100">
            {{ t('logout') }}
          </DisclosureButton>
        </div>
      </div>
