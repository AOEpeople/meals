<template>
  <div class="text-center">
    ...
    <h1 class="inline">{{ t('printList.title') + dateString }}</h1>
  </div>
          ...
          <div class="overflow-hidden aoe-shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tr class="divide-y divide-gray-200">
                <th scope="col" class="px-4 py-3.5 pl-4 text-left text-sm font-semibold text-primary"></th>
                <th v-for="mealName in mealNames" scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-primary">{{ mealName }}</th>
              </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
              <template v-for="(participant, slotName) in listData.data" :key="slotName">
                <tr class="border-t border-gray-200">
                  <th colspan="5" scope="colgroup" class="bg-gray-50 px-4 py-2 text-left text-sm font-semibold text-gray-900 sm:px-6">{{ slotName }}</th>
                </tr>
                <tr v-for="(participations, participantName, index) in participant" :class="[index === 0 ? 'border-gray-300' : 'border-gray-200', 'border-t']">
                  <td class="whitespace-nowrap py-4 pr-3 pl-4 text-sm font-medium text-primary sm:pl-6">{{ participantName }}</td>
                  <td v-for="participation in participations" class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500">
                    <svg v-if="participation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="m-auto block h-6 w-6 text-primary">
              ...
              <tr class="border-t border-gray-200">
                ...
                <th v-for="totalSum in listData.participations" scope="colgroup" class="bg-gray-50 px-4 py-2 text-center text-sm font-semibold text-gray-900 sm:px-6">
                  {{ totalSum }}
                </th>
              </tr>
              </tbody>
...
import {usePrintableListData} from "@/hooks/getPrintableListData";
