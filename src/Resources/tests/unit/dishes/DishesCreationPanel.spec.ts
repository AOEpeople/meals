...
describe('Test DishesCreationPanel', () => {
    ...
    it('should contain inputs with the correct values from the props', async () => {
        ...
        expect((wrapper.find('#dish\\.popover\\.descriptionDe').element as HTMLInputElement).value).toBe(Dishes[0].descriptionDe);
        expect((wrapper.find('#dish\\.popover\\.descriptionEn').element as HTMLInputElement).value).toBe(Dishes[0].descriptionEn);
        expect(wrapper.findComponent(CategoriesDropDown).vm.selectedCategory).toBe(getCategoryById(Dishes[0].categoryId));
